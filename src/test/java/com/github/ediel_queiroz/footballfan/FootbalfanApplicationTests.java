...
class FootbalfanApplicationTests {
    ...
    @Test
    void shouldConnectToKafkaSuccessfully(final CapturedOutput output) {
        kafkaTemplate.send("live-results", "test", samplePayload());
        Assertions.assertThatNoException().isThrownBy(() -> Awaitility.await().atMost(Duration.ofSeconds(1)).until(() -> output.getOut().contains("LiveScoreListener     : Receiving live results with key test")));
    }
